...
def draw_mre_chart(csv_path):
    ...
    plt.tight_layout()
    plt.savefig('mre_comparison_chart_gammav11.png', dpi=300)
    print("Đã xuất biểu đồ thành công: mre_comparison_chart_gammav11.png")
    plt.show()


def draw_comparison_chart(seq):
    df = pd.read_csv(f'logs/{seq}_details_gammav11.csv')
    ...
    plt.savefig(f'plots/comparison_chart_{seq}_gammav11.png')
    plt.show()

draw_mre_chart('final_results_gammav11.csv')
# for i in range(21):
