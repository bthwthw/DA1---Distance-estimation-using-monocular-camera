...
def draw_mre_chart(csv_path):
    ...
    plt.tight_layout()
    plt.savefig('mre_comparison_chart_gammav11_ema.png', dpi=300)
    print("Đã xuất biểu đồ thành công: mre_comparison_chart_gammav11_ema.png")
    plt.show()


def draw_comparison_chart(seq):
    df = pd.read_csv(f'logs/{seq}_details_gammav11_ema.csv')
    ...
    plt.savefig(f'plots/comparison_chart_{seq}_gammav11_ema.png')
    plt.show()

draw_mre_chart('final_results_gammav11_ema.csv')
for i in range(21):
    if i == 5 or i == 8 or i == 12 or i == 14:
        continue
    draw_comparison_chart(f"{i:04d}")
# draw_comparison_chart('0019')