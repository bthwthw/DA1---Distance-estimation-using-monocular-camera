...
def draw_mre_chart(csv_path):
    ...
    plt.tight_layout()
    plt.savefig('5_mre_comparison_chart.png', dpi=300)
    print("Đã xuất biểu đồ thành công: 5_mre_comparison_chart.png")
    plt.show()


def draw_comparison_chart(seq):
    df = pd.read_csv(f'logs/5_{seq}_details.csv')
    ...
    plt.savefig(f'plots/5_comparison_chart_{seq}.png')
    plt.show()

draw_mre_chart('5_final_results.csv')
for i in range(21):
