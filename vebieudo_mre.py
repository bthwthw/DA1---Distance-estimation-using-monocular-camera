...
def draw_mre_chart(csv_path):
    ...
    plt.tight_layout()
    plt.savefig('4_mre_comparison_chart.png', dpi=300)
    print("Đã xuất biểu đồ thành công: 4_mre_comparison_chart.png")
    plt.show()


def draw_comparison_chart(seq):
    df = pd.read_csv(f'logs/4_{seq}_details.csv')
    ...
    plt.savefig(f'plots/4_comparison_chart_{seq}.png')
    plt.show()

draw_mre_chart('4_final_results.csv')
# for i in range(21):
