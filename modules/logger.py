...
class SystemLogger:
    def __init__(self, sequence_name, log_dir="logs"):
        ...
        self.detail_file = os.path.join(self.log_dir, f"6_{sequence_name}_details.csv")
    ...
    def _init_detail_file(self):
        ...
        with open(self.detail_file, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['frame', 'cls_id', 'obj_id', 'dist_gt', 'dist_pred', 'error_pct', 'iou'])
    ...
    def save_csv(self, filename="final_results_6.csv"):
        s = self.get_summary()
